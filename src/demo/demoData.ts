...
import type {
  ...
  Employee,
  Weekday,
} from "../features/clubs/model/club.types";
...
export const DEMO: {
  ...
  clubs: [
    {
      ...
      cancellation_policy:
        "Free cancellation up to 12 hours before the booking.",
    ...
    },
    {
      ...
      description:
        "The city's biggest complex. Floodlit hard courts open till midnight.",
      website: "https://example.com",
      phone: "+35929887766",
  ...
  employees: {
    1: [
      { first_name: "Ivana", last_name: "Petrova" },
...
export function demoReservations(): DemoReservation[] {
  try {
    return JSON.parse(
      localStorage.getItem(LS.demoRes) || "[]",
    ) as DemoReservation[];
  } catch {
    return [];
  }
...
export function demoAvailability(courtId: number, date: string): Slot[] {
  ...
  const hours = (DEMO.openingHours[club.id] || []).find(
    (row) => row[0] === weekday,
  );
  if (!hours) return [];
  ...
  const booked = new Set(
    demoReservations()
      .filter(
        (reservation) =>
          reservation.court === courtId && reservation.date === date,
      )
  ...
  for (let minuteOfDay = openAt; minuteOfDay < closeAt; minuteOfDay += 30) {
    ...
    const price =
      (peak ? 11 : court.is_indoor ? 9 : 7) +
      (court.surface_type === "Grass" ? 3 : 0);
